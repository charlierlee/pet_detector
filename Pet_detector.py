...
my_number2 = os.environ['MY_DIGITS2']
...
def pet_detector(frame):
    ...
    if classes[0][0] in category_index.keys():
    ...
    if (display_str == 'cat' or display_str == 'dog' or display_str == 'bird') and pause == 0:
    ...
    if inside_counter > 4:
        ...
        message = client.messages.create(
            body = 'Your {pet} wants outside!'.format(pet=display_str),
            from_=twilio_number,
            to=my_number
            )
        message2 = client.messages.create(
            body = '{pet} requires your service, hooman!'.format(pet=display_str),
            from_=twilio_number,
            to=my_number2
            )
        inside_counter = 0
    ...
    if outside_counter > 4:
        ...
        message = client.messages.create(
            body = 'Your {pet} wants inside!'.format(pet=display_str),
            from_=twilio_number,
            to=my_number
            )
        message2 = client.messages.create(
...
if camera_type == 'picamera':
    ...
    footage_socket.connect('tcp://192.168.1.144:5555')
